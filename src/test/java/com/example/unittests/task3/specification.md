...
* **Description:** If the total quantity of all items combined in the cart is 3 or more, the single cheapest item unit price becomes completely free. This promotion must be applied before any percentage promo codes.
* **Acceptance Criteria:** A cart containing 1 `"Book"` (`50.00`), 1 `"Video Game"` (`120.00`), and 1 `"Pen"` (`10.00`) has a total quantity of 3. The `"Pen"` (`10.00`) becomes free, leading to a subtotal of `170.00`. Combined with `"SAVE10"`, the final total is: 170.00 * 0.90 = 153.00.
* **Implementation Tips:**
    - Calculate the total item count using `.mapToInt(CartItem::quantity).sum()`.
